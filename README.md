...
## 🚀 Build, Esecuzione e Release Cross-Platform (macOS & Windows)

### 💻 Prerequisiti di Sviluppo Locale
* **JDK 25** configurato nel PATH (`java -version`, `javac -version`, `jpackage --version`).
* **MariaDB Server** attivo in locale sulla porta 3306 con le credenziali definite in `application.yaml` (database `nepe_db`).
* **Maven Wrapper** incluso nel repository (`./mvnw`).

### 📦 Compilazione ed Esecuzione Locale
```bash
# Esecuzione della suite completa dei test deterministici
./mvnw clean test

# Compilazione ed avvio dell'applicazione desktop in locale
...
1. **Creare e inviare un tag Git:**
   ...
   ```
2. **Elaborazione Parallela su GitHub Actions:**
   * **Runner macOS (`macos-latest`):** compila il sorgente ed esegue `jpackage` generando l'immagine disco standalone **`NEPE-<versione>.dmg`** (con la classe principale `org.springframework.boot.loader.launch.JarLauncher`).
   * **Runner Windows (`windows-latest`):** compila scaricando le librerie native JavaFX per Windows ed esegue `jpackage` producendo l'installer setup **`NEPE-<versione>.exe`** con integrazione desktop e menu Start.
3. **Pubblicazione Release:** La GitHub Action crea la release ufficiale su GitHub allegando entrambi gli installer pronti per il download e generando le relative note di rilascio.
4. **Trigger Manuale (Workflow Dispatch):** È possibile generare una release anche direttamente dal browser dalla tab **Actions** di GitHub, selezionando *"Release Packages (macOS DMG & Windows EXE)"* e cliccando su *"Run workflow"*.
